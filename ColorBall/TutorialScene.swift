...
class TutorialScene: SKScene, SKPhysicsContactDelegate {
    ...
    // MARK: tutorial specific values
    
    var playerTappedLeft = false
    var playerTappedRight = false
    var notifiedTaps = false
    var notifiedStageOne = false

    var playerDidSpin = false
    var playerDidSpinFast = false
    var notifiedSpins = false
    var notifiedStageTwo = false
    
    var tutorialColumnsCleared = 0
    ...
    func checkStageOne() {
        if playerTappedRight, playerTappedLeft {
            if tutorialColumnsCleared <= 0, !notifiedTaps {
                ...
                NotificationCenter.default.post(name: Settings.TutorialStageOneCompletedNotification, object: nil, userInfo: nil)
            }
        }
    }
